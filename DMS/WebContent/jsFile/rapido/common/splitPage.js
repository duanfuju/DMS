...
var common_splitPage = function() {
	...
	var splitPageHtml = function(divId, formId, totalRow, pageSize, pageNumber, totalPages, isSelectPage, isSelectSize, orderColunm, orderMode, currentPageCount){
		...
		splitStr += '<ul  style="font-size:13px;" class="pagination pagination-blue">';
		...
		if(isSelectPage == true){
			splitStr += '&nbsp;&nbsp;<li><select name="pageNumber"  onChange="common_splitPage.splitPageLink(\''+divId+'\', \''+formId+'\', this.value);" style="width: 120px; height:30px;font-size:13px;">';
			for (var i = 1; i <= totalPages; i++) {
				if (i == pageNumber) {
		...
		if(isSelectSize == true){
			splitStr += '<li><select name="pageSize" onChange="common_splitPage.splitPageLink(\''+divId+'\', \''+formId+'\', 1);" style="width: 100px; height:30px;font-size:13px;">';
		...
		splitStr += '&nbsp;&nbsp;<li>显示<strong style="font-weight: 700;">' + start + '至<strong>' + end + '</strong>条，共<strong>' + totalRow + '</strong>' + i18n_common_splitPage_records + '</li>';
	...
	var splitPageLink = function(divId, formId, toPage){
		...
		if(divId == "mainPanel"){
			common_ajax.ajaxFormMainPanel(formId,undefined,undefined);
		}else{
