...
function autoComplete(inputId, autoShowId){
	var wordInput = $("#"+inputId);  //文本框
	var dataUrl = wordInput.attr("data-url");
    var inputOffset = wordInput.offset();//获取匹配元素在当前视口的相对偏移
    
    $("#"+autoShowId).hide().css("border", "1px black solid").css("position", "absolute").css("z-index",5).css("background-color","white")
    ...
    wordInput.keyup(function(event) {   //为文本框注册事件
        ...
        if ((keyCode >= 65 && keyCode <= 90) || keyCode == 8 || keyCode == 46 || keyCode==32 || (keyCode >= 96 && keyCode <= 105) || (keyCode >= 48 && keyCode <= 57))
        {
            ...
            //showData(inputId, autoShowId, data);
            $.ajax({
		    	  type : "post",
		    	  url : encodeURI(encodeURI(cxt + dataUrl)),
		    	  data : {keyword : wordText}, //, size : items
		    	  dataType : "text",
		    	  contentType: "application/x-www-form-urlencoded; charset=UTF-8",
		    	  async: false,
		    	  cache: false,
		    	  success: function(returnData){//alert(returnData);
		    		  //var data = eval('(' + returnData + ')');//['aa', 'bb', 'cc'];
		    	      showData(inputId, autoShowId, returnData);
		    	  },
            ...
            //$.post(dataUrl, {"autoText":wordText}, showData(data));
        //代表向上和向下键
        } else if (keyCode == 38 || keyCode == 40) {
            if (keyCode == 38){//向上
                var autoNodes = $("#"+autoShowId).children("div");
            ...
            if (heightShowIndex != -1)
            {
                var autoNodes = $("#"+autoShowId).hide().children("div");
                var comText = autoNodes.eq(heightShowIndex).text();//取出高亮显示的文本内容
                heightShowIndex = -1;
                $("#"+inputId).val(comText);
            } else {//下拉框没有值
                $("#"+autoShowId).hide();
                $("#"+inputId).get(0).blur();   //让文本框失去焦点
                //alert("文本中的[" + $("#"+inputId).val() + "]被提交了");
                //if($("#"+inputId).val().trim().length != 0){
                	//window.location.href = encodeURI(toPageUrl + $("#"+inputId).val());
                //}
            }
        }
    });
...
function showData(inputId, autoShowId, data){
	clearTimeout(timeOutId);//取消上次未完成的延迟
	setTimeout(new Function("showAutoSelect('"+inputId+"', '"+autoShowId+"', '"+data+"')"), 500);
}
			
function showAutoSelect(inputId, autoShowId, data) {
    var wordNodes = eval(data);
    var autoNode = $("#"+autoShowId); //找到页面的div
    ...
    $.each( wordNodes, function(i, n){//alert( "Item #" + i + ": " + n );
        ...
        newDivNode.mouseover(function() {
            if (heightShowIndex != -1)
            { //将原来高亮的节点取消高亮显示
                $("#"+autoShowId).children("div").eq(heightShowIndex).css("background-color", "white").css("color", "#000000");
            }
...
function autoCompleteHidden(autoShowId){
	var obj = document.getElementById(autoShowId);
