...
public abstract class BaseController extends Controller {
	...
	public void deleteImages(String imgs){
		if(imgs != null){			
	...
	protected String getWordPath(){
		String path = getRequest().getSession().getServletContext().getRealPath("");
		path = path+File.separator+ PropKit.get("tomcat.WordPath")+File.separator;
		return path;
	}
	
	
	protected String getExcelPathAndCreate(){
		String path = getRequest().getSession().getServletContext().getRealPath("")+File.separator+PropKit.get("tomcat.ExcelPath")+File.separator;
	 ...
	 * @throws Exception
	 */
	protected void baseCreateWord(Map<String, Object> params, String templateName, String fileName) throws Exception{
		baseCreateWord(params, templateName, fileName, "");
	}
	/**
	 * 生成word 文件
	 * @param params
	 ...
	 * @throws Exception
	 */
	protected void baseCreateWord(Map<String, Object> params, String templateName, String fileName,String folderName) throws Exception{
		String path = getRequest().getSession().getServletContext().getRealPath("");
		path = path+File.separator+ PropKit.get("tomcat.WordPath")+File.separator+folderName;//.substring(0,path.lastIndexOf("/")) 
		File file = new File(path);
		if(!file.exists()){
			file.mkdirs();
		}
		createWord(params, templateName, path + fileName);
	}
	
	private void createWord(Map<String, Object> params, String templateName, String path) throws Exception{
		WordUtil.create(params , templateName, path);
	}
}
