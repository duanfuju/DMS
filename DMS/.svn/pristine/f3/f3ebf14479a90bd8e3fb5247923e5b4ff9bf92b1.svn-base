...
public class RwTaskController2 extends BaseController {
	...
	public void chakan() throws Exception{
		...
		try {
			...
			List<Record> list_r=Db.find("SELECT "+
										     "   wx.fuZeRen fuZeRen,wx.weiXiuTime weiXiuTime "+
										"FROM xz_wx wx  "+
										"LEFT JOIN "+
										"		DMS_RW_task_info_relate tir  "+
										"				ON tir.ids=wx.taskInfoRelate  "+
										"LEFT JOIN "+
										"		DMS_XC_xunChaInfo xi  "+
										"				ON tir.xunChaInfoId = xi.ids  "+
										"LEFT JOIN "+
										"		DMS_RW_task rs  "+
										"				ON tir.taskId = rs.ids  "+
										"WHERE "+
										"		rs.ids='"+rwtask.get("ids")+"' "+
										"ORDER BY wx.weiXiuTime ");
			if (list_r!=null) {
				Record r=list_r.get(0);
				String r_date=r.getDate("weiXiuTime").toString();
				map.put("weiXiuTime_y", r_date.substring(0,4));
				map.put("weiXiuTime_m", r_date.substring(5,7));
				map.put("weiXiuTime_d", r_date.substring(8, 10));
				map.put("weiXiuTime_h", r_date.substring(11,13));
				map.put("weiXiuTime_min", r_date.substring(14,16));
