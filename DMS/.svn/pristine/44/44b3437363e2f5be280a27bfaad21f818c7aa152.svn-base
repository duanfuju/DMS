...
public class CheckInfoServiceImpl implements CheckInfoService {
	...
	public List<Map> getParentMenuMap() {
		...
		sql += " ( ";
		sql += " select a.ids,a.name,a.parentId from DMS_PZ_checkInfo a where EXISTS (select * from DMS_PZ_checkInfo ck where a.ids = ck.ids and ck.parentId is NULL and ck.isparent = 1  and ck.isdel = 0 )  ";
		sql += " union all ";
		sql += " select k.ids,k.name,k.parentId from DMS_PZ_checkInfo k inner join cte c on c.ids = k.parentId and k.parentId is NOT NULL and k.isparent = 1  and k.isdel = 0 ";
		sql += " )select * from cte ";
	...
	public List<Map> getTopParentMenuMap() {
		...
		String sql =" select * from DMS_PZ_checkInfo where parentId is NULL and isparent = 1 and  isdel = 0 ";
		List<Record> recordList = Db.find(sql);
		if(recordList != null && recordList.size() > 0)
		{
			for (int i = 0; i < recordList.size(); i++) {
