...
public class XunChaInfoService extends BaseService {
	...
	public Map<String, Object> getXunChaInfoByIds(String ids) {
		...
		Record record=Db.find("SELECT xunChaStartZHK,xunChaStartZHM,xunChaEndZHK,xunChaEndZHM,xiuBuBiaoZhi,wxmkId,binghaiOrQueXianType,binghaiOrQueXianMiaoShu,level," + 
				"yuGuGongChengLiang,yuGuGongChengLiangDw,xunChaTime,remark,imgs,xunChaUserId,xunChaTouId,luXian,wzType,wzName,wzMiaoShu,createTime,state,binghaiOrQueXianType,wxmkId from DMS_XC_xunChaInfo"
				+ " where ids='"+ids+"'").get(0);
		...
		map.put("isFZHL", false);
		//判断是否是防撞护栏     TODO:防撞护栏Id:1a435846fb7e42af9dadd1d1bcae9f38
		if(record.get("wxmkId").toString().equals("1a435846fb7e42af9dadd1d1bcae9f38"))
		{
			List<Record> list = Db.find(" select num,unit from DMS_XC_xunChaInfo_fzhl_relate where xunChaInfoId = '"+ids+"' order by num asc");
			String numUnit = "";
			for (int i = 0; i < list.size(); i++) {
				numUnit += list.get(i).get("num")+"("+list.get(i).get("unit")+"),";
			}
			numUnit = numUnit.substring(0,numUnit.lastIndexOf(","));
			map.put("yuGuGongChengLiang", numUnit);
			map.put("isFZHL", true);
		}
		return map;
	}
