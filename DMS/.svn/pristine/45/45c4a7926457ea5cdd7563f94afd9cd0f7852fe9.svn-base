...
import com.junl.dms.mvc.baoyan.WxView;
...
public class RwTaskController extends BaseController {
	...
	public void index() {
		if ((splitPage.getOrderColunm() == null || ("").equals(splitPage
				...
				&& (splitPage.getOrderMode() == null || ("").equals(splitPage
						.getOrderMode()))) {
			splitPage.setOrderColunm("taskNo");
	...
	public void add() {
		...
		UserInfo userinfo=UserInfo.dao.findById(user.get("userinfoids"));
		setAttr("singleMan", userinfo.getNames());
	...
	public void delete() {
		...
		try {
			String ids = getPara("ids");
			if (RwTaskService.service.deleteTaskBefore(ids)) {
				...
				jsonObject.put("result", false);
				jsonObject.put("result_desc", "此任务已经有关联的维修记录");
			}
	 ...
	 */
	public void choose() {
		String  userIds = getAttr(ConstantWebContext.request_cUserIds);//获取用户ID
		//根据用户ID查询该用户所在路段权限
		String sql=BaseService.service.getZHbyUserId(userIds,"x.xunChaStartZHK","x.xunChaStartZHM");
	...
	public void choose_checkBox() {
		List<WxView> list=BaoYanService.service.findAll();
		setAttr("list", list);
		render("/manage/baoyan/chooseCheckBox.html");
	}
