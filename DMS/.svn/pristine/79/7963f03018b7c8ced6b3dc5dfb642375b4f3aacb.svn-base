...
var common_ajax = function() {
	 ...
	 * @param callback 回调
	 */
	var ajaxMainPanel = function(url, data, callback , isTwo){
		var result = ajaxFunc(url, data, callback);
	...
	/**
	 * 使用FormData，进行Ajax请求并上传文件 求并返回结果 
	 * @param divId 返回替换div
	 * @param formId 提交formid
	 * @param callback 回调
	 */
	var ajaxFileForm = function(formId,dataType, data,callback){
		if(dataType == undefined || dataType == null){
			dataType = "html";
		}
		var result = "";
	 ...
	 * @param callback 回调
	 */
	var ajaxFormMainPanel = function(formId, callback,isTwo){
		var result = ajaxForm(formId, callback);
		var toolbarDiv = $("#toolbarDiv");
		layer.msg("操作成功！");
		if(isTwo){
